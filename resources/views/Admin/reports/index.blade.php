    ...
    <div class="space-y-8">
        ...
        <section class="rounded-[48px] border border-blue-200 bg-white p-8 shadow-lg">
                    ...
                    <tbody class="divide-y divide-blue-50 bg-white">
                        @forelse ($communityEmissions as $row)
                            <tr class="hover:bg-blue-50/70">
                                ...
                                <td class="px-6 py-4 text-slate-600">{{ $row['total_emission'] ?? '0' }}</td>
                                <td class="px-6 py-4 text-blue-700">{{ $row['average_monthly'] ?? '0' }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="mt-4 flex justify-end">
                @include('components.table-pagination', ['paginator' => $communityEmissions])
            </div>
        </section>
        ...
        <section class="rounded-[48px] border border-blue-200 bg-white p-8 shadow-lg">
                    ...
                    <tbody class="divide-y divide-blue-50 bg-white">
                        @forelse ($challengeParticipation as $row)
                            <tr class="hover:bg-blue-50/70">
                                ...
                                <td class="px-6 py-4 text-blue-700 font-semibold">{{ $row['points'] ?? 0 }} poin</td>
            ...
            <div class="mt-4 flex items-center justify-between">
                ...
                @include('components.table-pagination', ['paginator' => $challengeParticipation])
            </div>
